...
io.on('connection', socket => {
  ...
  // 🔒 Admin: Obtener salas activas
  socket.on('admin:get-active-rooms', async (data, ack) => {
    const { adminId } = data;
    if (!isAdminUser(adminId)) {
      logger.warn(
        `Intento de obtener salas activas por no admin: ${adminId ? adminId.slice(0, 6) + '...' : 'N/A'}`
      );
      return ack && ack({ ok: false, error: 'not_admin' });
    }
    ...
    try {
      ...
      // Convertir el formato de servers a rooms
      for (const [gameType, gameServers] of Object.entries(servers)) {
        for (const server of gameServers) {
          rooms.push({
            ...
            maxPlayers: server.maxPlayers,
            hasPassword: server.hasPassword,
            started: server.gameState?.started || false,
            gameType: gameType,
            createdAt: server.createdAt
          });
        }
      ...
      return ack && ack({ ok: true, rooms });
    } catch (e) {
      logger.error('Error obteniendo salas activas', e);
      return ack && ack({ ok: false, error: 'internal' });
    }
  });

  // 🔒 Admin: Terminar sala
  socket.on('admin:terminate-room', async (data, ack) => {
    const { adminId, roomId } = data;
    if (!isAdminUser(adminId)) {
      logger.warn(
        `Intento de terminar sala por no admin: ${adminId ? adminId.slice(0, 6) + '...' : 'N/A'}`
      );
      return ack && ack({ ok: false, error: 'not_admin' });
    }

    try {
      // Buscar la sala en impostor y CS16
      let room = null;
      let gameType = null;

      if (impostorRooms.has(roomId)) {
        room = impostorRooms.get(roomId);
        gameType = 'impostor';
      } else if (cs16Rooms.has(roomId)) {
        room = cs16Rooms.get(roomId);
        gameType = 'cs16';
      }
      ...
      if (gameType === 'impostor') {
        // Notificar a todos los jugadores que el juego terminó
        io.to(roomId).emit('impostor:game-ended', { reason: 'admin_terminated' });
        // Limpiar la sala
        impostorRooms.delete(roomId);
      } else if (gameType === 'cs16') {
        // Notificar a todos los jugadores que el juego terminó
        io.to(roomId).emit('cs16:game-ended', { reason: 'admin_terminated' });
        // Limpiar la sala
        cs16Rooms.delete(roomId);
      }

      logger.info(`Sala ${roomId} terminada por admin ${adminId ? adminId.slice(0, 6) + '...' : 'N/A'}`);
      return ack && ack({ ok: true });
    } catch (e) {
      logger.error('Error terminando sala', e);
      return ack && ack({ ok: false, error: 'internal' });
    }
  });

  // 🔒 Admin: Eliminar sala
  socket.on('admin:delete-room', async (data, ack) => {
    const { adminId, roomId } = data;
    if (!isAdminUser(adminId)) {
      logger.warn(
        `Intento de eliminar sala por no admin: ${adminId ? adminId.slice(0, 6) + '...' : 'N/A'}`
      );
      return ack && ack({ ok: false, error: 'not_admin' });
    }
    ...
    try {
      ...
      io.to(roomId).emit(`${gameType}:room-deleted`, { reason: 'admin_deleted' });
      ...
      logger.info(`Sala ${roomId} eliminada por admin ${adminId ? adminId.slice(0, 6) + '...' : 'N/A'}`);
      return ack && ack({ ok: true });
    } catch (e) {
      logger.error('Error eliminando sala', e);
      return ack && ack({ ok: false, error: 'internal' });
    }
  });

  // 🔒 Admin: Unirse a sala como observador
  socket.on('admin:join-room-as-admin', async (data, ack) => {
    const { adminId, roomId } = data;
    if (!isAdminUser(adminId)) {
      logger.warn(
        `Intento de unirse a sala como admin por no admin: ${adminId ? adminId.slice(0, 6) + '...' : 'N/A'}`
      );
      return ack && ack({ ok: false, error: 'not_admin' });
    }
    ...
    try {
      ...
      if (gameType === 'impostor') {
        socket.emit('impostor:joined-room', {
          roomId,
      ...
      logger.info(`Admin ${adminId ? adminId.slice(0, 6) + '...' : 'N/A'} se unió a sala ${roomId} como observador`);
      return ack && ack({ ok: true });
    } catch (e) {
      logger.error('Error uniéndose a sala como admin', e);
      return ack && ack({ ok: false, error: 'internal' });
    }
  });
