...
export const formatMessage = (content: string): string => {
  ...
  formatted = formatted.replace(
    urlRegex,
    '<a href="$1" target="_blank" rel="noopener noreferrer" class="text-discord-blurple hover:underline">$1</a>'
  );
  ...
  formatted = formatted.replace(
    mentionRegex,
    '<span class="text-discord-blurple font-medium">@$1</span>'
  );
  ...
  formatted = formatted.replace(
    /`([^`]+)`/g,
    '<code class="bg-[#1e1f22] px-1 py-0.5 rounded text-sm font-mono">$1</code>'
  );
