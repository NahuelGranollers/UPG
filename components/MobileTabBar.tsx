...
interface MobileTabBarProps {
  activeTab: 'channels' | 'chat' | 'users';
  onTabChange: (tab: 'channels' | 'chat' | 'users') => void;
  unreadCount?: number;
}

const MobileTabBar: React.FC<MobileTabBarProps> = memo(({ activeTab, onTabChange, unreadCount = 0 }) => {
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-discord-sidebar border-t border-gray-900/20 z-30 md:hidden safe-area-bottom">
      <div className="flex items-center justify-around h-16">
        ...
        >
          ...
          <Menu size={24} className={`transition-transform ${activeTab === 'channels' ? 'scale-110' : ''}`} />
        ...
        >
          ...
          <MessageSquare size={24} className={`transition-transform ${activeTab === 'chat' ? 'scale-110' : ''}`} />
        ...
        >
          ...
          <Users size={24} className={`transition-transform ${activeTab === 'users' ? 'scale-110' : ''}`} />
